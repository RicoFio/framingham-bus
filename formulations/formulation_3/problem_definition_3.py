...
class ProblemDefinitionTwo:
    graph: nx.DiGraph  # Directed road network graph with travel metrics on edges.

    Q_max: int  # Maximum number of rounds/trips allowed per bus.
    ...
    )  # Pickup stops set (inferred from graph).
    S: list[School] = field(default_factory=list)  # Schools set (inferred from graph).
    D: list[Depot] = field(
        default_factory=list
    )  # Physical depots set (inferred from graph).
    ...
    S_plus: list[str] = field(default_factory=list)  # School start-copy nodes.
    ...
    service_node_to_road_node: dict[str, NodeId] = field(
        default_factory=dict
    )  # Map from service-node label to road node id.
    service_node_kind: dict[str, str] = field(
        default_factory=dict
    )  # Service-node type tags.
    school_to_start_copy: dict[NodeId, str] = field(
        default_factory=dict
    )  # Map from school node id to its start-copy label.
    start_copy_to_school: dict[str, NodeId] = field(
        default_factory=dict
    )  # Map from start-copy label back to school node id.

    # Rounds
    Q: list[int] = field(default_factory=list)  # Round index set {1, ..., Q_max}.

    # School-time parameters
    h_s: dict[NodeId, float] = field(default_factory=dict)  # School bell/start times.
    delta_s: dict[NodeId, float] = field(
        default_factory=dict
    )  # Required slack before each school bell.
    T_bar: float = 0.0  # Global time horizon upper bound (also used as Big-M_time).

    def __post_init__(self) -> None:
        if not self.graph.is_directed():
            raise ValueError("ProblemDefinitionTwo.graph must be directed.")

        if self.Q_max < 1:
            raise ValueError("Q_max must be at least 1.")

        # Infer P, S, D directly from graph and validate school metadata strictly.
        self.P, self.S, self.D = self._infer_graph_sets(self.graph)

        self.Q = list(range(1, self.Q_max + 1))

        self.F = [student for student in self.M if student.is_sped]
        self.K = [student for student in self.M if student.is_monitor]

        stop_nodes = {stop.node_id for stop in self.P}
        school_nodes = {school.node_id for school in self.S}
        depot_nodes = {depot.node_id for depot in self.D}
        ...
        for student in self.M:
            if student.stop.node_id not in stop_nodes:
                raise ValueError(
                    f"Student {student.id!r} references stop node {student.stop.node_id!r}, "
                    "which is not present in graph stop set P."
                )
            if student.school.node_id not in school_nodes:
                raise ValueError(
                    f"Student {student.id!r} references school node {student.school.node_id!r}, "
                    "which is not present in graph school set S."
                )

    @staticmethod
    def _infer_graph_sets(
        graph: nx.DiGraph,
        ...
        validated_school_names: set[str] = set()
        school_meta_by_node = {school.node_id: school for school in schools}
        school_by_node: dict[NodeId, School] = {}
        for node_id, data in graph.nodes(data=True):
            ...
            if "name" not in data:
                raise ValueError(
                    f"School node {node_id!r} is missing required attribute 'name'."
                )

            school_name = str(data["name"]).strip()
            if not school_name:
                raise ValueError(
                    f"School node {node_id!r} has an empty required attribute 'name'."
                )

            if school_name in validated_school_names:
                raise ValueError(
                    f"Duplicate school name {school_name!r} detected. "
                    "School names must be unique."
                )

            validated_school_names.add(school_name)
            school_meta = school_meta_by_node[node_id]
            school_by_node[node_id] = School(
                name=school_name,
                node_id=node_id,
                x=school_meta.x,
                y=school_meta.y,
        ...
        schools = sorted(
            school_by_node.values(), key=lambda school: str(school.node_id)
        )
    ...
    def build_service_graph(self) -> None:
        ...
        self.school_to_start_copy = {}
        self.start_copy_to_school = {}
        ...
        for school in self.S:
            ...
            school_start_label = f"s+:{school.node_id}"
            self.S_plus.append(school_start_label)
            self.N.append(school_start_label)
            self.service_node_to_road_node[school_start_label] = school.node_id
            self.service_node_kind[school_start_label] = "school_start"
            self.school_to_start_copy[school.node_id] = school_start_label
            self.start_copy_to_school[school_start_label] = school.node_id
